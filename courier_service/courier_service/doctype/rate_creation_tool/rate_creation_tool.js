...
frappe.ui.form.on("Rate Creation Tool", {
    ...
    first_check_buying_rate : async function (frm) {
            ...
            {
                await frappe.call({
                    method: "courier_service.courier_service.api.api.first_check_buying_rate",
                    args: {
    ...
    create_rate : async function(frm, rate_type) {
            ...
            else if(frm.doc.service_type && frm.doc.full_tariff && frm.doc.icris_account && frm.doc.weight_slab.length )
            {
                ...
                if (valid) {
                        ...
                        await frappe.call({
                            method: "courier_service.courier_service.api.api.check_for_existing_rate",
                            args: {
                                full_tariff : frm.doc.full_tariff,
                                icris_account : frm.doc.icris_account,
                            ...
                            callback: function (r) {
                                if (r.message) {
                                    ...
                                    frappe.confirm('Rate already exist. Do you want to replace it ?',
                                        async () => {
                                            await frappe.call({
                                                method: "courier_service.courier_service.api.api.create_rate",
                                                args: {
                                                    full_tariff : frm.doc.full_tariff,
                                                    icris_account : frm.doc.icris_account,
                                                    weight_slab : frm.doc.weight_slab,
    ...
    service_type: async function(frm) {
        if (frm.doc.service_type) {
            ...
            await frappe.call({
                method: "courier_service.courier_service.api.api.get_full_tariff",
                args: {
   ...
   // refresh: async function(frm) {
    ...
    //                                     async () => {
    //                                         await frappe.call({
    //                                             method: "courier_service.courier_service.api.api.create_selling_rate",
    //                                             args: {
    //                                                 full_tariff : frm.doc.full_tariff,
    //                                                 icris_account : frm.doc.icris_account,
    //                                                 weight_slab : frm.doc.weight_slab,
